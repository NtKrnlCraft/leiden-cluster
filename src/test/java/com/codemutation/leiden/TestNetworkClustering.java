...
public class TestNetworkClustering
{
    ...
    public void setUp()
    {
        int[][] e = { { 0, 0, 1, 1, 1, 2, 2, 3, 3, 3, 4, 4, 5, 5, 5, 6, 6, 7 }, { 1, 2, 0, 2, 3, 0, 1, 1, 4, 5, 3, 5, 3, 4, 6, 5, 7, 6 } };
    ...
    public void testReducedNetworkQualityValue()
    {
        ...
        double expectedQuality = algorithm.calcQuality(testNetwork, clustering);
        ...
        double reducedQuality = algorithm.calcQuality(reducedNetwork, reducedClustering);
    ...
    public void testLeidenAlgorithm()
    {
        ...
        LeidenAlgorithm algorithm = new LeidenAlgorithm(resolution, LeidenAlgorithm.DEFAULT_N_ITERATIONS,
                LeidenAlgorithm.DEFAULT_RANDOMNESS, new Random(0));
        ...
        assertEquals((16 - 0.1 * (3 * 3 + 5 * 5)) / 18, algorithm.calcQuality(testNetwork, clustering), 1e-10);

        resolution = 0.5;
        algorithm = new LeidenAlgorithm(resolution, LeidenAlgorithm.DEFAULT_N_ITERATIONS,
                LeidenAlgorithm.DEFAULT_RANDOMNESS, new Random(0));
        ...
        assertArrayEquals(expectedClustering.getClusters(), clustering.getClusters());
        assertEquals((14 - resolution * (3 * 3 + 3 * 3 + 2 * 2)) / 18, algorithm.calcQuality(testNetwork, clustering),
                1e-10);
    }
}
